...
initial_velocity = lpm2ms(lpm = n2_lpm, diameter = 0.45)
# Defining ml/min 
mlmin1 <-
  needed_mlmin(
    wanted_ppl = ppl,
    flowrate = flow_lpm1+n2_lpm,
    initial_particle_diameter = initial_pm
  )
#Defining system RH
rh1 = rh_function(
  mass_water = mlmin1 - needed_mlmin(
    ppl,
    flowrate = flow_lpm1+n2_lpm,
    initial_particle_diameter = drydim
  ), # - the ml of dried particles that take up volume in a sphere
  volume_of_space = (rhflow * 0.001),
  temp = 293.15
) / 100
...
df <- tibble(time = seq(from = 0, to = 15, by = 0.001)) %>%
  ...
  mutate("flow_lpm" = flow_lpm1+n2_lpm) %>%
...
length_finaldim <- length(newpm[newpm > drydim])
newpm[newpm < drydim] = drydim
...
p <- ggplot(df3) +
  ...
  labs(
    ...
    caption = paste("Is the particle alwarys above 0cm \n and dry before it reaches the top of the column? =", truefalse,
                    ...
                    needed_concentration(wetdim = initial_pm, drydim = drydim),
                    " \n",
                    "needed ml/min = ",
                    round(
                      needed_mlmin(
                    ...
                    " \n",
                    "Initial velocity =",
                    round(lpm2ms(n2_lpm, diameter = 0.45), digits = 4),
                    " \n",
                    "Column m/s = ",
                    round(lpm2ms(flow_lpm1+n2_lpm, diameter = 10), digits = 3),
                    " \n",
                    "RH = ",
                    round(
                      rh_function(
...
particle_position(initial_pm = 55, 
                  flow_lpm1 = 25,
                  drydim = 20, 
                  n2_lpm = 5, 
                  ppl= 190000, 
